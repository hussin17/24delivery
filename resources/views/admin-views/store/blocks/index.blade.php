    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                @if (Request::query('block_id'))
                    <span>
                        {{ translate('edit_city') }} {{ isset($block->zone->name) ? $block->zone->name : '' }}
                    </span>
                @else
                    <span>
                        {{ translate('add_city') }} {{ isset($store_blocks->name) ? $store_blocks->name : '' }}
                    </span>
                @endif
            </h1>
        </div>
        <!-- End Page Header -->
        <form
            action="{{ Request::query('block_id') ? route('admin.store.blocks.update', Request::query('block_id')) : route('admin.store.blocks.store') }}"
                ...
                <div class="col-md-12">
                    <div class="delivery_charge_options" id="delivery_charge_options">
                        <div class="row gy-1" id="mod-label">
                            <div class="col-sm-4">
                                <label for="">{{ translate('messages.city Name') }}</label>
                            </div>
                            <div class="col-sm-4">
                        ...
                        <div class="row gy-1 module-row" id="module_">
                            <div class="col-sm-4">
                                <select class="form-control" placeholder="{{ translate('messages.choice_block') }}"
                                    name="block_id">
                                    @if (Request::query('block_id'))
                                        <option value="{{ $block->block_id }}">{{ $block->zone->name }}</option>
                                    @endif
                                    <option value="">{{ translate('messages.choice_city') }}</option>
                            ...
                            <div class="col-sm-4">
                                <input type="number" class="form-control" name="shipping_price"
                                    value="{{ $block->shipping_price ?? '' }}" step="0.001"
        ...
        <div class="table-responsive datatable-custom">
            <table id="columnSearchDatatable"
                ...
                <tbody id="set-rows">
                    @php($non_mod = 0)
                    @foreach ($store_blocks as $key => $block)
                        {{-- @php($non_mod = count($block->modules) > 0 && $non_mod == 0 ? $non_mod : $non_mod + 1) --}}
                        <tr>
                            <td>{{ $key + 1 }}</td>
                            <td>
                            ...
                            <td>
                                <div class="btn--container justify-content-center">
                                    <a class="btn action-btn btn--primary btn-outline-primary"
                                        href="{{ route('admin.store.blocks.index', ['store_id' => Request::query('store_id'), 'block_id' => $block['id']]) }}"
                                        title="{{ translate('messages.edit_city') }}">
                                        <i class="tio-edit"></i>
                                    </a>
                                    <a class="btn action-btn btn--danger btn-outline-danger status_form_alert"
                                        href="javascript:" data-id="city-{{ $block['id'] }}"
                                        data-title="{{ translate('Want_to_Delete_this_city?') }}"
                                        title="{{ translate('messages.delete_city') }}">
                                        <i class="tio-delete-outlined"></i>
                                    </a>
                                    <form action="{{ route('admin.store.blocks.destroy', [$block['id']]) }}" method="post"
                                        id="city-{{ $block['id'] }}">
                                        @csrf
                                        @method('DELETE')
                                    </form>
            ...
            @if (count($store_blocks) !== 0)
                <hr>
            @endif
            <div class="page-area">
                {!! $store_blocks->withQueryString()->links() !!}
            </div>
            @if (count($store_blocks) === 0)
...
@push('script_2')
    ...
    <script>
        ...
        let modules = <?php echo json_encode($store_blocks); ?>;
        let mod = {{ count($store_blocks) }};
        if (mod > 0) {
            $('#mod-label').show();
        } else {
            $('#mod-label').hide();
