...
@section('content')
    ...
    <div class="content container-fluid">
        ...
        <div class="page-header">
            <h1 class="page-header-title">
                ...
                @else
                    <span>
                        {{ translate('add_kms') }}
                        {{ isset($kms->from) && isset($kms->to) ? '( ' . $kms->from . ' - ' . $kms->to . ' )' : '' }}
                    </span>
                @endif
            </h1>
        </div>
        <!-- End Page Header -->
        <form
            action="{{ Request::query('id') ? route('admin.business-settings.kms.update', Request::query('id')) : route('admin.business-settings.kms.store') }}"
                ...
                <div class="col-md-12">
                    <div class="delivery_charge_options" id="delivery_charge_options">
                        ...
                        <div class="row gy-1 module-row" id="module_">
                            ...
                            <div class="col-sm-4">
                                <input type="number" step="0.001" class="form-control" name="shipping_price"
                            ...
                            <td>
                                <div class="btn--container justify-content-center">
                                    <a class="btn action-btn btn--primary btn-outline-primary"
                                        href="{{ route('admin.business-settings.kms.index', ['store_id' => Request::query('store_id'), 'id' => $item['id']]) }}"
                                    ...
                                    <form action="{{ route('admin.business-settings.kms.destroy', [$item['id']]) }}"
                                        method="post" id="kms-{{ $item['id'] }}">
                                        @csrf
                                        @method('DELETE')
                                    </form>
                                </div>
...
@push('script_2')
    ...
    <script>
        ...
        let modules = <?php echo json_encode($kms); ?>;
        let mod = {{ count($kms) }};
        if (mod > 0) {
            $('#mod-label').show();
        } else {
