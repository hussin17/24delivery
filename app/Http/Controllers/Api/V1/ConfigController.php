...
class ConfigController extends Controller
{
    ...
    /**
     * Calculates the extra charges for a given distance.
     *
     * @param Request $request The request object containing the distance parameter.
     * @throws \Illuminate\Validation\ValidationException If the distance parameter is missing.
     * @return \Illuminate\Http\JsonResponse The JSON response containing the extra charges.
     */
    ...
    public function autosuggestionmap($search)
    {
        $search .= ' , البحرين';
        $apiKey = 'rKdZVKfHAvnCHfGP5BoLt0SRhHxT-NMNoSdpXlHHpPk';
        // Make an API call to fetch latitude and longitude
        $response = Http::get("https://autosuggest.search.hereapi.com/v1/autosuggest", [
            'apiKey' => $apiKey,
            'q' => $search,
            'at' => "26.13868,50.56178"
        ]);
        ...
        if ($response->successful()) {
            ...
            if (request()->type == 'block') {
                ...
                foreach ($data['items'] as $key => $value) {
                    if (Str::contains(Str::lower($value['title']), 'البحرين')) {
                        array_push($res, $value);
                    }
                }
                return response(['data' => $res], 200);
            }
            return response(['data' => $data['items']], 200);
        } else {
            return response(['message' => translate('messages.response_failed')], 403);
        }
    }
}
