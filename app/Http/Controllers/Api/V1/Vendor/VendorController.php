...
class VendorController extends Controller
{
    public function get_profile(Request $request)
    {
        ...
        $vendor['Payable_Balance'] = (float) ($vendor?->wallet?->balance);
        ...
        if ($vendor['balance'] > 0) {
            ...
            $vendor['dynamic_balance'] = (float) abs($vendor?->wallet?->collected_cash);
        ...
        if ($store->transport_way == 0) {
            $vendor['vehicles'] = DMVehicle::active()->get();
        } else {
            // $vendor['vehicles'] = DMVehicle::active()->where('id', '=', $store->transport_way)->get();
            $vendor['vehicles'] = [];
        }
    ...
    public function get_items(Request $request)
    {
        ...
        $paginator = Item::with('tags')->type($type)->Approved()->where('store_id', $request['vendor']->stores[0]->id)->where('name', 'like', '%' . $search . '%')->latest()->paginate($limit, ['*'], 'page', $offset);
        $data = [
            'total_size' => $paginator->total(),
            'limit' => $limit,
            'offset' => $offset,
