...
class DeliverymanController extends Controller
{
    ...
    public function get_latest_orders(Request $request)
    {
        ...
        $distance = (int) BusinessSetting::where('key', 'search_delivery_distance')->first()->value;
        $myDistance = $distance;
        // DB::enableQueryLog();
        $orders = Order::with(['customer', 'store', 'parcel_category']);
        ...
        $orders = $orders->dmOrder() // order_type => delivery
            ->Notpos() // order_type <> pos
            ->NotDigitalOrder() // order_status <> pending
            ->OrderScheduledIn(30) // created_at <> schedule_at || schedule_at < now
        ...
        foreach ($orders as $order) {
            if ($order->created_by == 'store') {
                if ($order->distance <= ($distance * 3)) {
                    array_push($orderData, $order);
                }
            }
            if ($order->created_by == 'customer') {
                if ($order->zone_id == $dm->zone_id) {
                    array_push($orderData, $order);
                }
            }
        }
        // Sort Data By Distance
        $orders = Helpers::order_data_formatting($orderData, true);
        return response()->json($orders, 200);
    }
    ...
    public function order_payment_status_update(Request $request)
    {
        ...
        $dm = DeliveryMan::where(['auth_token' => $request['token']])->first();
        $order = Order::where(['delivery_man_id' => $dm['id'], 'id' => $request['order_id']])->dmOrder()->first();
        if ($order) {
