...
class Helpers
{
        ...
        if ($multi_data) {
            foreach ($data as $item) {
                if (isset($item['store'])) {
                    ...
                    $item['visiable_store_phone'] = $item['store']['visiable_store_phone'];
                    $item['min_delivery_time'] =  (int) explode('-', $item['store']['delivery_time'])[0] ?? 0;
                    $item['max_delivery_time'] =  (int) explode('-', $item['store']['delivery_time'])[1] ?? 0;
                    unset($item['store']);
                } else {
            ...
            if (isset($data['store'])) {
                ...
                $data['visiable_store_info'] = (string) $data['store']['visiable_store_phone'];
        ...
        try {
            ...
            if ($order->order_status == 'confirmed' && $order->order_type != 'take_away' && config('order_confirmation_model') == 'deliveryman' && $order->payment_method == 'cash_on_delivery') {
                if ($order->store->self_delivery_system) {
                    ...
                    self::send_push_notif_to_topic($data, "restaurant_dm_1" . $order->store_id, 'new_order');
                } else {
                    $data = [
                        'title' => translate('messages.order_push_title'),
                        'description' => translate('messages.new_order_push_description'),
            ...
            if (in_array($order->order_status, ['processing', ' ']) && $order->delivery_man) {
                $data = [
                    'title' => translate('messages.order_push_title'),
    ...
    public static function send_notification_from_store_to_delivery($deliveries, $order)
    {
        if ($order->scheduled == 0 && $order->order_status == 'confirmed') {
            $data = [
                'title' => translate('messages.order_push_title'),
                'description' => translate('messages.new_order_push_description'),
                'order_id' => $order['id'],
            ...
            if ($deliveries) {
                foreach ($deliveries as $delivery) {
                    self::send_push_notif_to_device($delivery['fcm_token'], $data);
                    DB::table('user_notifications')->insert([
                        'data' => json_encode($data),
    ...
    public static function get_delivery_by_distance(float $centerLat, float $centerLon)
    {
        $distance = (int) BusinessSetting::where('key', 'search_delivery_distance')->first()->value; // EX: 5
        while ($distance <= ((int) $distance * 3)) { // 5 <= (5 * 3)
            // I reversed lat, lon to resoleve problem
            $deliveriesData = DeliveryHistory::withinDistance($centerLon, $centerLat, $distance)
                ->join('delivery_men', 'delivery_histories.delivery_man_id', '=', 'delivery_men.id')
                ->groupBy('delivery_men.id')->latest('delivery_men.id')->get();
            if ($deliveriesData->isNotEmpty()) {
                return $deliveriesData;
            }
            $distance += $distance;
        }
        $deliveriesData = DeliveryHistory::join('delivery_men', 'delivery_histories.delivery_man_id', '=', 'delivery_men.id')
            ->groupBy('delivery_men.id')->latest('delivery_men.id')->get();
