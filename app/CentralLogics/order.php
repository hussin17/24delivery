...
class OrderLogic
{
    ...
    public static function create_transaction($order, $received_by = false, $status = null)
    {
        ...
        $dm_tips_manage_status = BusinessSetting::where('key', 'dm_tips_status')->first()->value; // 1
        $admin_subsidy = 0;
        $amount_admin = 0;
        $store_d_amount = 0;
        ...
        if ($type == 'parcel') {
            ...
            $comission = isset($order->store->comission) == null ? \App\Models\BusinessSetting::where('key', 'admin_commission')->first()->value : $order->store->comission; // 20
            $dm_tips = $dm_tips_manage_status ? $order->dm_tips : 0;
            // $order_amount = $order->order_amount - $order->delivery_charge - $order->total_tax_amount - $dm_tips;

            if ($order->created_by == 'store') {
                $comission = 0;
            }
            ...
            $order_amount = $order->order_amount - $order->additional_charge - $order->delivery_charge - $order->total_tax_amount - $dm_tips + $flash_admin_discount_amount + $order->coupon_discount_amount + $store_discount_amount + $flash_store_discount_amount; // 20
            // comission in delivery charge
            $delivery_charge_comission = BusinessSetting::where('key', 'delivery_charge_comission')->first();
            $delivery_charge_comission_percentage = $delivery_charge_comission ? $delivery_charge_comission->value : 0; // 30
            $comission_on_delivery = $delivery_charge_comission_percentage * ($order->original_delivery_charge / 100); // 30 * (20 / 100) = 6

            if ($order->store->self_delivery_system) {
                $comission_on_actual_delivery_fee = 0;
            } else {

                $comission_on_actual_delivery_fee = ($order->delivery_charge > 0) ? $comission_on_delivery : 0; // 6
            }

            //final comission
            $comission_on_store_amount = ($comission ? ($order_amount / 100) * $comission : 0); // (20 / 100) * 20 = 4
            $comission_amount = $comission_on_store_amount + $comission_on_actual_delivery_fee; // 4 + 6 = 10
            $dm_commission = $order->original_delivery_charge - $comission_on_actual_delivery_fee; // 20 - 6 = 14
        ...
        // $store_amount = 20 + 0 - 4 - 0 - 0 = 20 - 4 = 16;
        try {
            OrderTransaction::insert([
                'vendor_id' => $type == 'parcel' ? null : $order->store->vendor->id,
                'delivery_man_id' => $order->delivery_man_id,
                'order_id' => $order->id,
                'order_amount' => $order->order_amount, // 40
                'store_amount' => $type == 'parcel' ? 0 : $store_amount, // 16
                // 'store_amount'=>$type=='parcel' ? 0 : $order_amount + $order->total_tax_amount - $comission_on_store_amount,
                'admin_commission' => $comission_amount + $order->additional_charge - $admin_subsidy - $admin_coupon_discount_subsidy, // 10
                'delivery_charge' => $order->delivery_charge, // 20
                'original_delivery_charge' => $dm_commission, // 14
